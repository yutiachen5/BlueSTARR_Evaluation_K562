...
def summary_statistics(X, Y, set, taskNum, numTasks, taskName, model):
    ...
    if (config.useCustomLoss) :
        ...
        df = pd.DataFrame({'true':naiveTheta,'predicted': pred.squeeze()})
        df.to_csv('/work/igvf-pm/K562/mdl_eval/slurms/outputs/K562-custom.txt', index = False, sep='\t')
    else:
        cor=stats.spearmanr(tf.math.exp(pred.squeeze()),tf.math.exp(Y))
        df = pd.DataFrame({'true':tf.math.exp(Y.numpy().ravel()),'predicted': tf.math.exp(pred.squeeze())})
        df.to_csv('/work/igvf-pm/K562/mdl_eval/slurms/outputs/K562-mse.txt', index = False, sep='\t')
