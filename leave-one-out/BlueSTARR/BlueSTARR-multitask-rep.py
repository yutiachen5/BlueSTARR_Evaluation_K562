...
def main(configFile,subdir,modelFilestem):
    ...
    training_set = [x for x in config.TrainData]
    (X_train_sequence, X_train_seq_matrix, X_train, Y_train) = \
        prepare_input(training_set,subdir,shouldRevComp,config.MaxTrain,config,'train')

    validation_set = [x for x in config.ValidationData]
    (X_valid_sequence, X_valid_seq_matrix, X_valid, Y_valid) = \
        prepare_input(validation_set,subdir,shouldRevComp,config.MaxTrain,config,'validation')
    ...
    with open(modelFilestem+config.TestData[0]+".json","w") as json_file:
        json_file.write(model_json)
    model.save_weights(modelFilestem+config.TestData[0]+".h5")
...
def summary_statistics(X, Y, set, taskNum, numTasks, taskName, model):
    ...
    if (config.useCustomLoss) :
        ...
        cor=stats.spearmanr(tf.math.exp(pred.squeeze()),Y)
...
def makeClosure(taskNum):
    ...
    def loss(y_true, y_pred):
        global EPSILON
        DNA=y_true[:,0:y_true.shape[1]//2]
        RNA=y_true[:,y_true.shape[1]//2:y_true.shape[1]]
        sumX=tf.reduce_sum(DNA,axis=1)
...
def loadFasta(fastas_file_list, as_dict=False,uppercase=False, stop_at=None,
    ...
    print('number of fasta seq: ', len(fastas))
...
def loadCounts(counts_file_list,maxCases,config):
    linesRead=0 # sum rna sum dna, mse tf built in mse function, no need for closure
    lines=[]
...
def prepare_input(dataset,subdir,shouldRevComp,maxCases,config, set_name):
    ...
    # validation data goes the same format as testing data
    if set_name == 'train':
        fastas_file_list = [subdir+"/"+x+'-train.fasta' for x in dataset]
        counts_file_list = [subdir+"/"+x+"-train-counts.txt" for x in dataset]  
    else:
