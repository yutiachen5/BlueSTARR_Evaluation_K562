...
#=========================================================================
if(len(sys.argv)!=6):
    exit(ProgramName.get()+" <in:fasta> <in:counts.txt.gz> <desired-sample-size> <out-dir> <train|test|validation>")
...
# outDiscardedFastaFile=outDir+"/all-validation.fasta.gz"
# outDiscardedCountFile=outDir+"/all-validation-counts.txt.gz"
# outDiscardedFastaFile=outDir+"/train-0.9-discarded.fasta.gz"
# outDiscardedCountFile=outDir+"/train-0.9-discarded.txt.gz"
# outDiscardedFastaFile=outDir+"/train-validation.fasta.gz"
# outDiscardedCountFile=outDir+"/train-validation-counts.txt.gz"
outDiscardedFastaFile=outDir+"/all-train.fasta.gz"
outDiscardedCountFile=outDir+"/all-train-counts.txt.gz"
...
for line in IN_COUNTS:
    ...
    if(random.random()<=alpha):
        ...
        print(line,end="",file=OUT_DISCARDED_COUNTS)
        fastaWriter.addToFasta(defline,sequence,OUT_DISCARDED_FASTA)

IN_COUNTS.close(); OUT_COUNTS.close(); OUT_FASTA.close(); OUT_DISCARDED_FASTA.close(); OUT_DISCARDED_COUNTS.close()